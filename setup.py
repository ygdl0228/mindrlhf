...
def get_description():
    ...
    return 'mindrlhf platform: %s, cpu: %s' % (os_info, cpu_info)
...
class EggInfo(egg_info):
    ...
    def run(self):
        egg_info_dir = os.path.join(os.path.dirname(__file__), 'mindrlhf.egg-info')
...
class BuildPy(build_py):
    ...
    def run(self):
        mindspore_transformer_lib_dir = os.path.join(os.path.dirname(__file__), 'build', 'lib', 'mindrlhf')
...
class Install(install):
    ...
    def run(self):
        ...
        if sys.argv[-1] == 'install':
            ...
            mindspore_transformer_dir = os.path.join(os.path.dirname(pip.__path__[0]), 'mindrlhf')
...
if __name__ == '__main__':
    ...
    setup(
        ...
        package_data={'mindrlhf': get_configs_content()},
        cmdclass={
            'egg_info': EggInfo,
            'build_py': BuildPy,
            'install': Install,
